...
class Car(models.Model):
    ...
    owner = models.ForeignKey(Profile, null=True, blank=True,
                              on_delete=models.CASCADE, related_name='owned_cars')
    ...
    fuel_type = models.CharField(
        max_length=20, choices=FUEL_CHOICES, blank=True, null=True)
    transmission_type = models.CharField(
        max_length=20, choices=TRANSMISSION_CHOICES, blank=True, null=True)
    ...
    custom = models.CharField(
        max_length=20, choices=CUSTOM_CHOICES, blank=True, null=True)
    body_type = models.CharField(
        max_length=20, choices=BODY_CHOICES, blank=True, null=True)
    color = models.CharField(max_length=10, blank=True,
                             null=True, choices=COLOR_CHOICES)
    ...
    tags = models.ManyToManyField(
        'Tag', related_name='tags', default=None, blank=True)
    tech_and_safety_features = models.ManyToManyField(
        'TechAndSafetyFeatures', related_name='cars', blank=True, default=None)
    maintenance_centers = models.ManyToManyField(
        MaintenanceCenter, default=None, blank=True, related_name='maintenance_centers')
    ...
    class Meta:
        constraints = [
            models.CheckConstraint(
                check=models.Q(release_year__gte=1900) & models.Q(
                    release_year__lte=current_year + 2),
...
class PriceHistory(models.Model):
    car = models.ForeignKey(Car, on_delete=models.CASCADE,
                            related_name='price_history', null=True)
...
class InteriorImage(models.Model):
    ...
    interior_image = models.ImageField(
        upload_to=car_image_interior_path, null=True, blank=True)
...
class ExteriorImage(models.Model):
    ...
    exterior_image = models.ImageField(
        upload_to=car_image_exterior_path, null=True, blank=True)
    uploaded_at = models.DateTimeField(auto_now_add=True)
...
class Review(models.Model):
    ...
    user = models.ForeignKey(
        Profile, on_delete=models.CASCADE, related_name='UserReviews')
    post = models.ForeignKey(
        Car, null=True, on_delete=models.CASCADE, related_name='reviews')
